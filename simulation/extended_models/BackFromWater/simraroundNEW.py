...
DT = 10	# 10, 20, 50, 100, 200. DT=Drinking Time, rats stay at the target for DT time step
# 		# then continue to explore the maze with TurnToWater probability at every junction
#		# to get back to the target
TurnToWater = 0.5	# Chance of turning back to Water at every junction
...
def step_one(r, g, rats, goal, n, h, m, s, vis, step):
	...
	rats[r]['steps_remaining'] -= 1
        ...
        if (rats[r]['poz']==goal) and (step-g.vs[goal]['last_visit'][r])<=DT:	# only stay at goal for DT steps, otherwise move as usual
	    go_to=rats[r]['poz']
	    rats[r]['steps_remaining']=0
	else:
	    rnd = random()
	    # decide on turning back to waterwards, if the agent already located it:
	    if (rnd <= TurnToWater) and (rats[r]['poz']<num_ver(level-1)) and (rats[r]['poz']>num_ver(0)) and g.vs[goal]['first_visit'][r]>0:
		go_to = g.vs[rats[r]['poz']]["towards_goal"]
		#print "step: " + str(step) + " rat: " + str(r) + " at: " + str(rats[r]['poz']) + " turns to water towards: " + str(go_to)
	    else:
	...
	if go_to!=rats[r]['poz']:
	    ...
	    if g.vs[rats[r]['poz']]['first_visit'][r] == 0:
		g.vs[rats[r]['poz']]['first_visit'][r] = step + rats[r]['steps_remaining']
...
out='n'+str(n)+'_l'+str(level)+'_h'+str(h)+'_m'+str(m)+'_s'+str(s)+'_vis'+str(vis)+'_DT'+str(DT)+'TurnToWater'+str(TurnToWater)
with open(out+'.txt', "w") as myfile:
	...
	# store for each node the time of the last visit by each rat
        ...
        g.vs["first_visit"]=[[0]*n for vert in range(len(g.vs))]
	...
	#print "goal: " + str(goal)
	# store for each node with other neighboring node leads toward the goal
        #g.vs["towards_goal"]=range(len(g.vs))
	g.vs[goal]["towards_goal"]=goal
	# first go "up" in the hierarchy to define direction from the route
	for ilevel in range(1,level):
	    for vid in range(num_ver(level-ilevel-1),num_ver(level-ilevel)):
		for v in g.successors(vid):
		    if g.vs[v]["towards_goal"] is not None:
			#print "here, vid: " + str(vid) + " towards_goal, v: " + str(v)
			g.vs[vid]["towards_goal"] = v
	# then go back downwards on the hierarchy, and define those node that are not set yet
	for ilevel in range(0,level):
	    for vid in range(num_ver(ilevel-1),num_ver(ilevel)):
		# if the direction from this node is not defined yet 
		#print "Defined here?, vid: " + str(vid) + " tovards_goal: " + str(g.vs[vid]["towards_goal"])
		if g.vs[vid]["towards_goal"] is None:
		    for v in g.neighbors(vid):
			#print "-> v " + str(v) + " g.vs[v][\"towards_goal\"]: " + str(g.vs[v]["towards_goal"])
			if g.vs[v]["towards_goal"] is not None:
			    #print "here, vid: " + str(vid) + " tovards_goal, v: " + str(v)
			    g.vs[vid]["towards_goal"] = v
	...
	if(write_traj):
	    ...
	    # step
            ...
            # if all arrived, end
            if min(g.vs[goal]['first_visit'])>0: ##len(g.vs[goal]['here_are'])==n:
                break
            #if step % (stepmax/10) ==0:
                #print 'actual step: ', step
        arrive = sorted(g.vs[goal]['first_visit'])
        arrives.append(arrive)
        #print str(arrives)
        out_line = "goal_first_visit: " + str(arrive)
        out_line += "\tstat:\t"
